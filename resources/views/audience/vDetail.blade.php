...
<!-- ปุ่มเปิดหน้าต่างลอย -->
<a class="add-to-cart" id="panoramaLink" onclick="panoramaClick()">Panorama 360°</a>
...
function panoramaClick() {
    ...
    if (!isLoggedIn) {
        // เรียกใช้ confirm เพื่อแสดงหน้าต่างลอยเตือน
        var agree = confirm('กรุณาเข้าสู่ระบบเพื่อดูภาพ Panorama');
        if (agree) {
            // ถ้าผู้ใช้กดตกลง ให้ redirect ไปยังหน้า login
            window.location.href = "{{ route('login') }}";
        } else {
